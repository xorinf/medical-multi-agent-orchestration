  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const newMsg = await messageService.sendMessage({
        ...
        sender_role: 'patient',
        recipient_id: doctorPartner.id,
        recipient_name: doctorPartner.name,
        content: input.trim(),
      });
      setMessages(prev => [...prev, newMsg]);
      setInput('');
      toast('Clinical message dispatched to Dr. Vance', 'success');
    } catch (err: any) {
      ...
      <Card className="flex-1 flex flex-col p-0 overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-charcoal-100 dark:border-charcoal-800 flex items-center justify-between bg-charcoal-50/50 dark:bg-charcoal-900/50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-sage-700 text-white flex items-center justify-center font-bold text-sm">
              CV
            </div>
            <div>
              <h2 className="text-sm font-bold text-charcoal-900 dark:text-ivory-100 flex items-center gap-2">
                {doctorPartner.name}
                <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold bg-sage-100 dark:bg-sage-900/40 text-sage-800 dark:text-sage-300">
                  Primary Physician
                </span>
              </h2>
              <p className="text-xs text-charcoal-500">
                {doctorPartner.specialty} · Board Certified
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-4">
          {loading ? (
            ...
            <div className="py-12 text-center text-xs text-charcoal-400">
              No direct messages yet. Send a query to your physician below.
            </div>
          ) : (
            messages.map(m => {
                ...
                >
                  <div
                    className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 text-xs font-bold ${
                      isMe ? 'bg-charcoal-800 text-white' : 'bg-sage-700 text-white'
                    }`}
                  >
                    {isMe ? 'You' : 'MD'}
                  </div>
        ...
        <form onSubmit={handleSend} className="p-4 border-t border-charcoal-100 dark:border-charcoal-800 flex gap-2">
          <input
            ...
            placeholder={`Message ${doctorPartner.name}...`}
