...
class LocalGuardrails:
    ...
    def check_output(self, output: str, user_input: str = "") -> str:
        ...
        Returns the ORIGINAL output unchanged. The guardrail review is logged
        for audit but never replaces the user-facing answer (which was the
        previous bug — guardrails LLM was returning its own review text instead
        of the real answer).
        """
        if not output:
            return output
        ...
        # ponytail: a guardrail failure must never break the user-visible answer.
        try:
            review = self.output_guardrail_chain.invoke({
                "output": output_text,
        ...
        # If the review explicitly blocks/refuses, prepend a caution line and
        # still keep the original answer so the user gets something useful.
        if isinstance(review, str) and any(k in review.lower() for k in ["block", "refuse", "cannot provide"]):
            return (f"{output_text}\n\n"
                    f"_Note: this response was flagged for review by our safety layer. "
                    f"Please consult a qualified clinician before acting on it._")
        return output_text